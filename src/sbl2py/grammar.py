...
def make_decl(kw, targets, cls):
	declaration = Suppress(kw) + Suppress('(') + ZeroOrMore(NAME) + Suppress(')')
...
DECLARATION = MatchFirst([
	...
	make_decl(EXTERNALS, [state.externals, state.routines], ExternalDeclarationNode),
...
INT_LITERAL.setParseAction(parse_action(lambda t: IntegerLiteralNode(int(t[0]))))
...
EXPRESSION = operatorPrecedence(
	EXPRESSION_OPERAND,
	[
		(Suppress('-'), 1, opAssoc.RIGHT, make_node_action(NegationNode, ungroup=True)),
...
def make_int_cmd(op, cls):
	...
	# We're not using ``Combine`` here because ``Combine`` automatically converts
...
UNARY_OPERATOR = NOT | TEST | TRY | DO | FAIL | GOTO | GOPAST | REPEAT | BACKWARDS | (LOOP + EXPRESSION) | (ATLEAST + EXPRESSION)
...
GROUPING_DEF = Suppress(DEFINE) + GROUPING_REF + GROUPING_ATOM + ZeroOrMore(oneOf('+ -') + GROUPING_ATOM)
...
PROGRAM_ATOM = DECLARATION | ROUTINE_DEF | GROUPING_DEF | STRINGESCAPES_CMD | STRINGDEF_CMD
BACKWARD_SECTION = add_node_action(Suppress(BACKWARDMODE + "(") + ZeroOrMore(PROGRAM_ATOM) + Suppress(')'), BackwardModeNode)
PROGRAM = add_node_action(ZeroOrMore(PROGRAM_ATOM | BACKWARD_SECTION) + StringEnd(), ProgramNode)
